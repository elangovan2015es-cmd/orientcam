/* OrientCam sw.js v2.0 — orientcam-v2.0-cache — © EIL */
const CACHE='orientcam-v2.0-cache';
...
self.addEventListener('activate',e=>{e.waitUntil(caches.keys().then(keys=>Promise.all(keys.filter(k=>k!==CACHE).map(k=>caches.delete(k)))).then(()=>self.clients.claim()).then(()=>self.clients.matchAll().then(cs=>cs.forEach(c=>c.postMessage({type:'SW_UPDATED',version:'2.0'})))));});
